...
contract("Lottery", accounts =>{
    ...
    describe("Constructor", ()=>{
        it("Owner should be sets to accounts[0]", async()=>{
    ...
    describe("Enter", ()=>{
        ...
        it("Enter 5 players and check values", async()=>{
            ...
            assert.deepEqual(await lottery.getPlayer(), [accounts[1]]);

            expect((await lottery.getBalance()).toString()).to.equal(enterAmt);
            expect(await lottery.getPlayer()).to.deep.equal([accounts[1]]);

            ((await lottery.getBalance()).toString()).should.equal(enterAmt);
            (await lottery.getPlayer()).should.deep.equal([accounts[1]]);
            ...
            assert.deepEqual(await lottery.getPlayer(), [accounts[1], accounts[2]]);
            ...
            assert.deepEqual(await lottery.getPlayer(), [accounts[1], accounts[2],accounts[3]]);
            ...
            assert.deepEqual(await lottery.getPlayer(), [accounts[1], accounts[2],accounts[3],accounts[4]]);
            ...
            assert.deepEqual(await lottery.getPlayer(), [accounts[1], accounts[2],accounts[3],accounts[4], accounts[5]]);
        })
    })
    describe("PickWinner", ()=>{
        ...
        it("PickWinner", async ()=>{
            ...
            const winner = await lottery.winnerHistory[lotteryId-1];
